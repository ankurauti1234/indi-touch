...
const OWM_API_KEY = 'YOUR_EXISTING_OPENWEATHERMAP_API_KEY';
...
export function renderScreensaverMembers() {
    ...
    if (tvTag) {
        if (!config.bleAvailable) {
            ...
            if (clock) {
                clock.classList.remove('massive');
            }
        } else {
    ...
    if (watchingArea) {
        ...
        watchingArea.style.display =
            (tvState.on && activeMembers.length > 0) ? 'block' : 'none';
    }
